...
estadop *apadreI,*apadreF;			//Apuntador para saber el padre mas reciente
...
estadop generaHijo(int id,int a){
	int i,j;
	estadop hijo;
	generados++;
	if(a)
		 hijo.profundidad = apadreI->profundidad +1;
    else
    	 hijo.profundidad = apadreF->profundidad +1;
	...
	for(i=0; i<filas; i++)
		for(j=0; j<columnas; j++){
	        if(a)
				 hijo.puzzle[i][j] = apadreI->puzzle[i][j];
			else
				 hijo.puzzle[i][j] = apadreF->puzzle[i][j];
		}
