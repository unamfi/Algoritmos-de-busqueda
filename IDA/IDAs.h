...
//Pila agenda = NULL;			//Lista donde se almacenan los edos explorados
Pila listaS = NULL;			//lista donde se almacenan los edos que llevan a la solucion
int expandidos=0,res=0;			//Se almacena el numer de edos expandidos
Listad agenda = NULL;
...
estadop As(estadop edo, estadop final, int profundidad_corte){
	if(agenda == NULL){
		expandir(edo);
	}
	if(profundidad_corte>=0){
		if(compara_edos(edo,final)){
			...
			if(expandido(edo)){
				As(BorrarPrim(&agenda), final,profundidad_corte-1);
			}
			else{
				expandir(edo);
				As(BorrarPrim(&agenda), final,profundidad_corte-1);
...
estadop IDAs(estadop edo, estadop final){
	...
	while(!compara_edos(generic,final)){
		As(edo, final,profundidad_corte);
		if(profundidad_max > profundidad_corte){
			profundidad_corte++;
			profunSol = profundidad_corte;
		}
		else{
